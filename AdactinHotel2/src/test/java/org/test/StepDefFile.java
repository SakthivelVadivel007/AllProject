...
public void user_is_on_the_adactin_hotel_login_page() throws IOException {
	...
	@When("User should enter invalid {string}  and valid {string}")
	public void user_should_enter_invalid_and_valid(String string3, String string4) {

		implicitylWait(10);
		POM.getInstnce().getUsername().sendKeys(string3);
		POM.getInstnce().getPassword().sendKeys(string4);



	}

	@When("User should not navigate to search hotel page")
	public void user_should_not_navigate_to_search_hotel_page() {

	System.out.println("user should receive login is unsuccess message");
